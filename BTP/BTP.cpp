...
//****************************************************************************************************
Mat src_col1,src_col2,cameraMat,distMat(1,4,CV_64F),out1,out2;
int scale1,scale2;
//****************************************************************************************************
...
{
	distMat.at<double>(0,0) = -pow((double)10,-scale1) * scale2;
	//distMat.at<double>(0,1) = 0.00000000000000001;
...
int main()
{
	...
	//***************************************************** set variable ******************************************
	double overlap = 0.7; 
	double distanceTollerence= 0.02,slopeTollerence = 0.01;
	//***************************************************** set variable ******************************************
	Mat camMat(3,3,CV_64F);
	camMat = 0;
	camMat.at<double>(0,0) = 1;
	camMat.at<double>(0,2) = 1;
	camMat.at<double>(1,1) = 1;
	camMat.at<double>(1,2) = 1;
	...
	Mat tempImage;
	//***********************************************************************************************************************************************
	tempImage = Tool::getInstance()->GetEdgeScheleton(imgPath1,lowThreshold,ratio,kernel_size,BWThreshold);
	//junction_pts1 = Tool::getInstance()->getJunctionPoints(tempImage,BWThreshold); // set 1  of new feature points
	edges1 = Tool::getInstance()->GetCurvature(tempImage,k,curveLenThreshold,BWThreshold);	// k =30 :: the last edge returned contains the junction points
	...
	//***********************************************************************************************************************************************
	// right image
	tempImage = Tool::getInstance()->GetEdgeScheleton(imgPath2,lowThreshold,ratio,kernel_size,BWThreshold); 
	vector<point> junction_pts2;
	
	edges2 = Tool::getInstance()->GetCurvature(tempImage,k,curveLenThreshold,BWThreshold);	// k =30 :: the last edge returned contains the junction points
